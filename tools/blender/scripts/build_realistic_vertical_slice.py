...
def build_realistic_airport_terminal():
    ...
    # Materials (Bright Sunny Daytime Airport)
    m_tiles = create_mat("Mat_AirportFloorTiles", (0.75, 0.77, 0.80, 1.0), metallic=0.08, roughness=0.20)
    m_glass = create_mat("Mat_CurtainWallGlass", (0.65, 0.80, 0.95, 0.65), metallic=0.20, roughness=0.05)
    m_mullion = create_mat("Mat_SteelMullions", (0.25, 0.28, 0.32, 1.0), metallic=0.85, roughness=0.35)
    m_truss = create_mat("Mat_RoofTrussSteel", (0.45, 0.48, 0.52, 1.0), metallic=0.75, roughness=0.38)
    m_counter = create_mat("Mat_CheckinCounter", (0.88, 0.90, 0.92, 1.0), roughness=0.25)
    m_screen = create_mat("Mat_MonitorScreen", (0.20, 0.55, 0.95, 1.0), emission=(0.20, 0.55, 0.95, 1.0), emission_strength=2.5)
    m_xray = create_mat("Mat_XrayScanner", (0.35, 0.38, 0.42, 1.0), metallic=0.4, roughness=0.50)
    m_led = create_mat("Mat_SecurityLED", (0.2, 0.9, 0.3, 1.0), emission=(0.2, 0.9, 0.3, 1.0), emission_strength=3.0)
    m_seat = create_mat("Mat_AirportSeat", (0.20, 0.35, 0.55, 1.0), roughness=0.55)
    m_pillar = create_mat("Mat_PillarConcrete", (0.68, 0.70, 0.72, 1.0), roughness=0.75)
    m_luggage = create_mat("Mat_LuggageLeather", (0.55, 0.20, 0.15, 1.0), roughness=0.50)
    m_fluorescent = create_mat("Mat_FluorescentLight", (0.98, 0.99, 1.0, 1.0), emission=(0.98, 0.99, 1.0, 1.0), emission_strength=3.5)
    m_blood = create_mat("Mat_FloorBloodDecal", (0.55, 0.05, 0.04, 1.0), metallic=0.1, roughness=0.25)
